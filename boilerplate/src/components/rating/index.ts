...
export class CaseRating extends LitElement {
  ...
  getStarClassName(i: number): string {
    let isStarRated = i < this.indexOfRatedStar;
    if (isStarRated) {
      return this.isComponentDisabled ? 'star_disabled' : 'star_is_rated';
    } else {
